...
//  MemberManagement
//
//  Created by 503-17 on 07/12/2018.
...
class DataSync{
    ...
    func downloadData(){
        ...
        //guard는 조건에 맞지 않으면 작업을 수행하지 않도록
        //하기 위해서 사용하는 경우가 많습니다.
        guard userDefaults.value(forKey:"download") == nil else{
            return
        }
        
        //다운로드 받을 서버의 url 만들기
        let url = "http://192.168.0.116:8080/server/memo/memolist"
        //요청 보내기
        //let get = Alamofire.request(url, method:.get, encoding:JSONEncoding.default, headers:nil)
        
        //euckr
        let get = Alamofire.request(url, method:.get, encoding:URLEncoding.default, headers:nil)
        ...
        get.responseJSON{
            ...
            for item in list{
                ...
                //CoreData의 자료형인 MemoMO 객체 만들기
                let object = NSEntityDescription.insertNewObject(
                    forEntityName:"Memo", into:self.context)
                    as! MemoMO
                //데이터를 저장
                ...
                if let imagePath = record["IMAGE_PATH"] as? String{
                    if imagePath != " "{
                        let url = URL(string:"http://192.168.0.116:8080/server/memoimage/\(imagePath)")
